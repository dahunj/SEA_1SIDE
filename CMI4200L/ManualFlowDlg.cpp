...
/*
	if (m_chkReadRfid2.GetCheck()) {
		CRfidManager *pRfid = CRfidManager::Get_Instance();

		m_stcLotId2.SetWindowText("");
		m_stcPartNo2.SetWindowText("");

		Set_Rfid("");
		pRfid->Read_Rfid(RFID_CH2, RFID_LOT_ID);	// Lot ID
		Wait_Time_Rfid(100);
		m_stcLotId2.SetWindowText(m_strRfid);

		Set_Rfid("");
		pRfid->Read_Rfid(RFID_CH2, RFID_PART_NO);	// Part No
		Wait_Time_Rfid(100);
		m_stcPartNo2.SetWindowText(m_strRfid);

		m_chkReadRfid2.SetCheck(FALSE);
	}
*/
}
...
void CManualFlowDlg::Wait_Time_Rfid(int msec)
{
	CCommon *pCommon = CCommon::Get_Instance();
